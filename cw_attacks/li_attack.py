...
class CarliniLi:
    def __init__(self, sess, model,
                 ...
                 const_factor = CONST_FACTOR, verbose = True):
        """
...
#         const = tf.placeholder(tf.float32, [])
        ...
        def doit(oimgs, labs, starts, tt, CONST):
            ...
            boxmul = 1 / 2.
            boxplus = 1 / 2.
            imgs = np.arctanh((np.array(oimgs) - boxplus) / boxmul *0.999999)
            starts = np.arctanh((np.array(starts) - boxplus) / boxmul *0.999999)
            ...
            while CONST < self.LARGEST_CONST:
                ...
                for step in range(self.MAX_ITERATIONS):
                    ...
                    const = CONST
                    def attack_loss():
                        global output, loss1, loss2, loss, orig_output
                        newimg = tf.tanh(modifier + simg) * boxmul + boxplus
        
                        output = model.predict(newimg)
                        orig_output = model.predict(tf.tanh(timg) * boxmul + boxplus)
                        ...
                        # sum up the losses
                        loss2 = tf.reduce_sum(tf.maximum(0.0,tf.abs(newimg-(tf.tanh(timg) * boxmul + boxplus))-tau))
                        loss = const*loss1+loss2
                        return loss                                                            
                    # perform the update step
                    optimizer.minimize(attack_loss, var_list=[modifier])
                    if step%(self.MAX_ITERATIONS//10) == 0:
                        if(self.verbose):
                            print(step,loss,loss1,loss2)
                    works, scores = loss, output
#                     _, works, scores = sess.run([train, loss, output], feed_dict=feed_dict)
                    ...
                    if works < .0001*CONST and self.ABORT_EARLY:
                        newimg = tf.tanh(modifier + simg) * boxmul + boxplus       
                        get = model.predict(newimg)
                        works = compare(np.argmax(get), np.argmax(labs))
                        if works:
                            scores, nimg = get, newimg    
                            origscores = model.predict(tf.tanh(timg) * boxmul + boxplus)
#                             scores, origscores, nimg = sess.run((output,orig_output,newimg),feed_dict=feed_dict)
                            l2s=np.square(nimg-(np.tanh(imgs) * boxmul + boxplus)).sum(axis=(1,2,3))
    ...
    def attack_single(self, img, target):
        ...
        while tau > 1./256:
            ...
            res = self.grad(np.asarray([np.copy(img)]), np.asarray([target]), np.copy(prev), tau, const)
            if res == None:
