...
class CarliniL2:
    def __init__(self, model, batch_size=1, confidence = CONFIDENCE,
                 ...
                 boxmin = -0.5, boxmax = 0.5, verbose = True):
        """
        ...
        self.model = model
        image_size, num_channels, num_labels = model.input_shape[1], model.input_shape[-1], model.output_shape[-1]
...
#         self.train = self.optimizer.minimize(self.attack_loss, var_list=[modifier])
...
#         self.setup.append(self.const.assign(self.assign_const))
    ...
    def attack_loss(self):
        ...
        # prediction BEFORE-SOFTMAX of the model
#         self.output = self.model.predict(self.newimg)
...
#         imgs = np.arctanh((imgs - self.boxplus) / self.boxmul * 0.999999)
        imgs = tf.atanh((imgs - self.boxplus) / self.boxmul * 0.999999)
...
#                                        self.assign_const: CONST})
            self.timg.assign(batch)
            self.tlab.assign(batchlab)
            self.const.assign(CONST)
...
#                                                          self.newimg])
                self.optimizer.minimize(self.attack_loss, var_list=[self.modifier])
                ...
                if self.verbose == True and iteration%(self.MAX_ITERATIONS//10) == 0:
#                     print(iteration,self.sess.run((self.loss,self.loss1,self.loss2)))
                    print(iteration,self.loss,self.loss1,self.loss2)
